...
class Notifier:
    ...
    def __init__(self):
        ...
        if os.path.exists('./data_/tickers.pkl') and os.path.exists('./data_/users.pkl'):
            ...
            alarm_class = self.alarm_instances['rsi'](self.tickermanager.tickers[ticker.lower()])
            self.tickermanager.tickers[ticker.lower()].add_alarm_key('rsi', alarm_class)
    ...
    def process_alarm(self, tick_stat, ticker, df):
        alarm_list = self.tickermanager.tickers[ticker.lower()].get_alarm_key()
        for alarm_name in alarm_list:
            try:
                alarm = self.tickermanager.tickers[ticker.lower()].get_alarm_item(alarm_name)
    ...
    def process_bot(self, tick_stat, ticker, df):
        bot_list = self.tickermanager.tickers[ticker.lower()].get_bot_key()
        for bot_name in bot_list:
            try:
                bot = self.tickermanager.tickers[ticker.lower()].get_bot_item(bot_name)
